...
export function FolderContent({ folderId }: { folderId: string }) {
  ...
  // Find folder from context first (faster, already loaded)
  useEffect(() => {
  ...
  // Filter items by folder_id
  // Special case: "default" folderId means items with folder_id = null
  const filteredItems = items
    .filter((item) => folderId === "default" ? item.folder_id === null : item.folder_id === folderId)
    .filter((item) => {
    ...
    <DashboardLayout>
      <div className="min-h-full bg-white">
        ...
        <div className="px-6 py-4">
          {/* Folder Title */}
          <div className="mb-4">
            <h1 className="text-lg font-semibold text-[#1C1C1C]">{folderId === "default" ? "default" : (folder?.name ?? "Loading...")}</h1>
          </div>
                        ...
                        <td colSpan={7} className="px-4 py-16 text-center">
                          <div className="flex flex-col items-center justify-center">
                            ...
                            <p className="text-[#6B5B4F] mb-2 text-sm">
                              {searchQuery ? "No results found" : "No records in this folder"}
                            </p>
                          </div>
                        </td>
                      </tr>
                              ...
                              {openMenu === item.id && (
                                <div className="absolute right-0 top-full mt-1 w-36 bg-white rounded-lg shadow-lg border border-[#E8E0D5] py-1 z-20">
                                  ...
                                  <button
                                    ...
                                    className="w-full flex items-center gap-2 px-3 py-2 text-sm text-[#B8860B] hover:bg-[#FAF7F2] transition-colors"
                                  >
                                    <svg width="14" height="14" viewBox="0 0 24 24" fill={isStarred(item.id) ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2">
                                      <polygon points="12,2 15.09,8.26 22,9.27 17,14.14 18.18,21.02 12,17.77 5.82,21.02 7,14.14 2,9.27 8.91,8.26" />
                                    </svg>
