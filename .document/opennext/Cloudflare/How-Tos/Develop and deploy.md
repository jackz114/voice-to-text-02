...
While deploy will start serving your application as soon as it is uploaded, upload only creates a new version of the application so that you can use gradual deployments.

Local build
Use the build command followed by either deploy or upload to deploy your local build.

When running the build command locally, .dev.vars and Next .env files might override your configuration. It is preferable to use a CD system as Workers Builds to deploy your application for reproducible deployments.

Workers Builds
When using Workers Builds, make sure to setup your environment variables as explained in this guide.

You can then connect your GitHub repository by following the documentation.
