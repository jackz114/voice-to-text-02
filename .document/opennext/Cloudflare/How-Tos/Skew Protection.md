Skew protection
The Cloudflare adapter has experimental support for skew protection based on the preview URLs.

Preview URLs are disabled for Workers that implement a Durable Object. If your app uses Durable Objects, they will need to be implemented in a separate Worker.
...
Note that in this context, environment variables include both your local Cloudflare environment variables and the process' environment variables. If a variable is set in both places, the process environment variable will take precedence.
