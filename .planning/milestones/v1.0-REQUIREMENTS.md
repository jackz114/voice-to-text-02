# Requirements Archive: v1.0 AI Learning Assistant MVP
...
- [x] **TRANS-02**: 处理 Whisper API 25MB 文件大小限制（低比特率录制 + UI 时长限制）
...
- [x] **NOTIFY-01**: 系统在复习节点到达时主动提醒用户（Cloudflare Cron Trigger 每小时检查）
- [x] **NOTIFY-02**: 每日最多一次提醒，仅当有复习条目时发送
- [x] **NOTIFY-03**: 提醒消息包含当天需要复习的条目数量和领域
- [x] **NOTIFY-04**: 支持邮件作为提醒渠道（Resend API）

### Search

- [x] **SEARCH-01**: 用户可以用自然语言搜索自己的知识库（PostgreSQL tsvector 全文搜索）
- [x] **SEARCH-02**: 搜索结果按相关性排序（ts_rank）
- [x] **SEARCH-03**: 搜索结果显示知识条目的预览和来源（ts_headline 高亮摘要）
...
- **SEM-01**: 语义搜索（pgvector 向量相似度）— 数据库已预留 embedding 列和 HNSW 索引
...
|-------------|-------|--------|
| AUDIO-01 | Phase 2 — Review Loop | Complete |
| AUDIO-02 | Phase 2 — Review Loop | Complete |
| AUDIO-03 | Phase 2 — Review Loop | Complete |
| TRANS-01 | Phase 2 — Review Loop | Complete |
| TRANS-02 | Phase 2 — Review Loop | Complete |
| TRANS-03 | Phase 2 — Review Loop | Complete |
...
| NOTIFY-01 | Phase 3 — Retention Engine | Complete |
| NOTIFY-02 | Phase 3 — Retention Engine | Complete |
| NOTIFY-03 | Phase 3 — Retention Engine | Complete |
| NOTIFY-04 | Phase 3 — Retention Engine | Complete |
| SEARCH-01 | Phase 3 — Retention Engine | Complete |
| SEARCH-02 | Phase 3 — Retention Engine | Complete |
| SEARCH-03 | Phase 3 — Retention Engine | Complete |
