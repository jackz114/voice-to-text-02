...
export async function GET(request: NextRequest) {
  try {
    ...
    const results = await db
      ...
      .orderBy(desc(sql`ts_rank(${knowledgeItems.searchVector}, to_tsquery('chinese', ${searchQuery}))`))
      .limit(params.limit);
          ...
          <Container className="mx-auto max-w-[600px] p-6">
            <Section className="bg-white rounded-lg p-8 shadow-sm">
              ...
              <Text className="text-gray-700 text-lg mb-4">
                你有 <strong className="text-blue-600">{count}</strong> 个知识点正在等待加固，
                花 {Math.ceil(count * 0.5)} 分钟搞定它们吧！
              </Text>
              ...
              <Text className="text-gray-500 text-sm text-center">
                <a href="https://bijiassistant.shop/settings/notifications" className="text-blue-600 underline">
                  调整提醒设置
                </a>
                {" | "}
                <a href="https://bijiassistant.shop/unsubscribe?token=xxx" className="text-gray-400 underline">
                  退订
                </a>
              </Text>
            </Section>
