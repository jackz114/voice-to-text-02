...
- Improvement path: Cache the token in Cloudflare KV (already configured in `wrangler.toml`) with a TTL slightly below the token expiry.
...
- Current capacity: The `compatibility_flags: ["nodejs_compat_v2"]` flag in `wrangler.toml` enables Node.js compatibility shims.
